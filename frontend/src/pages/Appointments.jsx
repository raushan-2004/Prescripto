...
const Appointments = () => {
  ...
  // Fixed: Initialize as null instead of "null"
  ...
  const getAvailableSlots = async () => {
    // Fixed: Add guard clause to prevent errors
    if (!docInfo || !docInfo.slots_booked) {
      console.log('docInfo or slots_booked not available yet');
      return;
    }

    // Fixed: Reset docSlots to prevent accumulation
    setDocSlots([])
    ...
    for(let i = 0; i < 7; i++){
      ...
      // Fixed: Set end time to 21 hours (9 PM) instead of 24
      ...
      while(currentDate < endTime){
        ...
        // Fixed: Add proper null checks and fix the logic
        const isSlotAvailable = docInfo.slots_booked[slotDate] && docInfo.slots_booked[slotDate].includes(slotTime) ? false : true

        // Fixed: Push AVAILABLE slots (when isSlotAvailable is true)
        if(isSlotAvailable) {
          timeSlots.push({
        ...
        <div className='flex gap-3 items-center w-full overflow-x-scroll  mt-4'>
          {
            docSlots.length && docSlots.map((item,index ) =>(
              <div onClick={() => setSlotIndex(index)} className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${slotIndex === index ? 'bg-indigo-500 text-white' : 'border border-gray-600' }`}  key={index} >
